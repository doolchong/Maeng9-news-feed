...
public class BoardService {
    ...
    public String boardLike(long boardId, long userId) {
        ...
        return boardLikeRepository.findByBoard_IdAndUser_Id(boardId, userId)
                .map(boardLike -> { // 값이 있을 때
                    boardLikeRepository.delete(boardLike);
                    return "좋아요를 취소했습니다.";
                })
                .orElseGet(() -> {  // 값이 없을 때
                    boardLikeRepository.save(new BoardLike(findBoardById(boardId), userService.findByUserId(userId)));
                    return "좋아요를 했습니다.";
                });
    }
    ...
    public Page<BoardResponse> getHotBoardList(int page, int size) {
        ...
        return boardRepository.findByCreatedAtAfterOrderByBoardLikeCount(expirationDate, pageable)
                .map(BoardResponse::new);
    }
}
